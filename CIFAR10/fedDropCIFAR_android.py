...
class FedDropCIFARAndroid(Strategy):
    ...
    def __init__(
        ...
        # all shapes are 1-d when sent back from Android clients, hence we need to reshape it when sent back to server
        self.weight_shapes = [(3, 3, 3, 32), (32,), (3, 3, 32, 32), (32,),(3, 3, 32, 64), (64,),(3, 3, 64, 64), (64,),(3, 3, 64, 128), (128,),(3, 3, 128, 128), (128,), (2048, 512), (512,), (512, 256), (256,), (256, 10), (10,)]
    ...
    def configure_fit(
        ...
        for client in clients:
            ...
            if (client.cid in self.straggler) and rnd > 2:
                # Select the Dropout technique here from : drop_rand, drop_order, and drop_dynamic (Invariant dropout)
                fit_ins_drop = FitIns(self.drop_dynamic(parameters, self.p_val, [0, 2,4,6,8,12,14], 10, client.cid), config_drop)
                clientList.append((client, fit_ins_drop))
                print("sending dropout")
            else:
    ...
    def aggregate_fit(
        ...
        weights = self.aggregate_drop(weights_results, self.droppedWeights, self.weight_shapes,
                                      self.droppedWeightsShape)

        # sort the clients based on training duration (at round2 )
        for elem in results:
            print(elem[0].cid)
            print(elem[1].metrics.get('duration'))
        if (len(self.straggler) == 0) and rnd > 1:
            ...
            self.straggler[results[len(results) - 1][0].cid] = results[len(results) - 1][1].metrics.get('duration')
            ...
            if (slowest[0].cid not in self.straggler ):
                ...
                for i in range(len(results)):
                    if results[i][0].cid in self.straggler:
                        self.straggler[results[i][0].cid] = (results[i][1].metrics.get('duration') / self.p_val) * 1.15
                        print("Updated estimate straggler orig time to:", self.straggler[results[i][0].cid])
                ...
                if (slowest[1].metrics.get('duration') > stragglerList[0][1]):
                    self.straggler[slowest[0].cid] = slowest[1].metrics.get('duration')
    ...
    def drop_rand(self, parameters: Parameters, p: float, idxList: List[int], idxConvFC: int, cid: str):
        ...
        for idx in idxList:
            ...
            # ie for convolutional layers it is the number of filters, for FC layers its the number of output neurons
            ...
            list = random.sample(fullList, numToDrop)
            list.sort()
        ...
        # Dropping for the last convolutional layer (has to be flattened for FC layer)
        ...
        listFC: Tuple = random.sample(range(1, shape[first]), shape[first] - int(p * shape[first]))
        listFC.sort()
        ...
        # drop the neurons from each weight parameter ( # filters for conv layer, bias, # of inputs for first FC layer)
    ...
    def drop_dynamic(self, parameters: Parameters, p: float, idxList: List[int], idxConvFC: int, cid: str):
        ...
        for idx in idxList:
            ...
            if len(self.defDropWeights[idx]) >= numToDrop:
                ...
                dropList = random.sample(self.defDropWeights[idx], numToDrop)
                dropList.sort()
                ...
                for x in self.defDropWeights[idx]:
                    fullList.remove(x)
                dropList = random.sample(fullList, numToDrop - len(self.defDropWeights[idx]))
                ...
                for x in self.unchagedWeights[idx]:
                    fullList.remove(x)
                dropList = random.sample(fullList, numToDrop - len(self.unchagedWeights[idx]))
    ...
    def find_stable(self, parameters: Parameters, results: List[Tuple[Weights, int]], idxList: List[int],
                    idxConvFC: int):
        ...
        # Since each layer has its own threshold, we chekc if parameters fall under the threshold individually
        for idx in idxList:
            difference = []
            for i in range(len(weights)):
            ...
            for cWeights, num_examples, cid in results:
                ...
                clientWeights[0] = np.reshape(clientWeights[0], self.weight_shapes[0])
                clientWeights[2] = np.reshape(clientWeights[2], self.weight_shapes[2])
                clientWeights[4] = np.reshape(clientWeights[4], self.weight_shapes[4])
                clientWeights[6] = np.reshape(clientWeights[6], self.weight_shapes[6])
                clientWeights[8] = np.reshape(clientWeights[8], self.weight_shapes[8])
                clientWeights[10] = np.reshape(clientWeights[10], self.weight_shapes[10])
                clientWeights[12] = np.reshape(clientWeights[12], self.weight_shapes[12])
                clientWeights[14] = np.reshape(clientWeights[14], self.weight_shapes[14])
                clientWeights[16] = np.reshape(clientWeights[16], self.weight_shapes[16])

                for i in range(len(weights)):
                    difference[i] += (np.abs(clientWeights[i] - weights[i]) <= np.abs(
                        self.changeThreshold[idx] * weights[i])) * 1

            # We treat weight parameter as "Invariant" only if its an "invariant" weight parameter for at least 75% of the non-straggler clients
            ...
            # Calculate which neuron is "invariant" for all related weight parameters related to that neuron
            ...
            # Check whcih neurons were dropped last round and is still in the unchangedlist
        ...
        for i in range(len(idx0Layer)):
            ...
            reducedList = np.array(np.append(reducedList, np.all(idx2Layer[i * 4 * 4:  i * 4 * 4 + 4 * 4])),
                                   dtype=bool)
    ...
    def find_min(self, parameters: Parameters, results: List[Tuple[Weights, int]], idxList: List[int], rnd: int):
        ...
        # For each layer, calculate the % change of each neuron based on the maximum % change of its related weight parameters
        for idx in idxList:
            dim = [x for x in range(weights[idx].ndim)]
            dim.remove(weights[idx].ndim - 1)
            ...
            # Take average min values of neurons as initial update Threshold value
            if (rnd == 2):
                self.changeThreshold[idx] = sum[noChangeIdx[0]]
                print("threshold for ", idx, "updated to: ", self.changeThreshold[idx])
            if (rnd == 3):
                self.changeThreshold[idx] = (self.changeThreshold[idx] + sum[noChangeIdx[0]]) / 2
                print("threshold for ", idx, "updated to: ", self.changeThreshold[idx])

    def aggregate_drop(self, results: List[Tuple[Weights, int, str]], dropWeights: Dict[str, List], origWeightsShape: List,
                       dropWeightsShape: Dict[str, List]) -> Weights:
        ...
        for i in range(len(origWeightsShape)):
            total_examples_wDrop.append(np.full(origWeightsShape[i], num_examples_total))

        # transform the list of weights into original format
        # We will expand sub-models to the global model shape by filling in 0s for dropped weights
        transformedResults = []
        for (clientWeights, num_examples, cid) in results:
            ...
            if cid not in dropWeights:
                for i in range(len(origWeightsShape)):
                    # since Android clients send weights in 1 Dimension, we reshape it for easier calculation
                    if transformedWeights[i].shape != origWeightsShape[i]:
                        transformedWeights[i] = np.reshape(transformedWeights[i], origWeightsShape[i])
            ...
            for i in range(len(origWeightsShape)):
                ...
                if transformedWeights[i].shape != dropWeightsShape[cid][i]:
                    transformedWeights[i] = np.reshape(transformedWeights[i], dropWeightsShape[cid][i])
            ...
            for [row, col] in dropWeights[cid]:
                ...
                # for each row that's dropped add a row in the weight parameter with all 0s
                if (rowLen != 0):
                    transformedWeights[layer] = np.insert(transformedWeights[layer], row - np.arange(len(row)), 0,
                                                          axis=transformedWeights[layer].ndim - 1)
                    # since the row was dropped, the neuron did not train with this client's data
                    # Hence remove client's data count from total examples trained for related weights.
                    total_examples_wDrop[layer][..., row] -= num_examples

                # for each row that's dropped add a row in the weight parameter with all 0s
                if (colLen != 0):
                    transformedWeights[layer] = np.insert(transformedWeights[layer], col - np.arange(len(col)), 0,
                                                          axis=transformedWeights[layer].ndim - 2)
                    # since the colum was dropped, the neuron did not train with this client's data
                    # Hence remove client's data count from total examples trained for related weights.
                    total_examples_wDrop[layer][..., col, :] -= num_examples

                    # Check if any number of examples for any weights were subtracted twice if both its row and col was dropped.
            ...
            # Append the transformed client model to the result list, with number of examples that each individual weight trained on.
            transformedResults.append((transformedWeights, num_examples))
