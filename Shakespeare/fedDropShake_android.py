...
class FedDropShakeAndroid(Strategy):
    ...
    def configure_fit(
        ...
        for client in clients:
            ...
            if (client.cid in self.straggler) and rnd > 2:
                ...
                fit_ins_drop = FitIns(self.drop_dynamic(parameters, self.p_val, [0,3], 10, client.cid), config_drop)
    ...
    def aggregate_fit(
        ...
        # Calculate the invariant weights, and current smallest weight % different
            ...
            if (slowest[0].cid not in self.straggler):
                ...
                for i in range(len(results)):
                    if results[i][0].cid in self.straggler:
                        self.straggler[results[i][0].cid] = results[i][1].metrics.get('duration') / self.p_val
    ...
    def drop_rand(self, parameters: Parameters, p: float, idxList: List[int], idxConvFC: int, cid: str):
        # Note: for each LSTM layer in the model, it has related weight parameters in 4 indices:
    ...
    def drop_order(self, parameters: Parameters, p: float, idxList: List[int], idxConvFC: int, cid: str):
        ...
        for idx in idxList:
            ...
            # save a copy of the dropped neurons
            list = [x for x in range(shape[second_1] - numToDrop, shape[second_1])]
            self.prevDropWeights[idx] = list.copy()
            print("Dropped weights idx ", idx, ": ", (self.prevDropWeights[idx]))
    ...
    def find_stable(self, parameters: Parameters, results: List[Tuple[Weights, int]], idxList: List[int],
        ...
        # For each client that trained on the full model, calculate which weight parameters have a change below the threshold
        for cWeights, num_examples, cid in results:
            ...
            clientWeights[1] = np.reshape(clientWeights[1], self.weight_shapes[1])
            clientWeights[3] = np.reshape(clientWeights[3], self.weight_shapes[3])
        ...
        for idx in idxList:
            ...
            # we need to check whether a neuron's parameters for all 4 matrices fall under the threshold
            reduced0List = np.array([])
            reduced10List = np.array([])
            reduced2List = np.array([])
            for i in range(len(idx11Layer)):

                reduced0List = np.array(np.append(reduced0List, (idx0Layer[i] and idx0Layer[i+128] and idx0Layer[i+256] and idx0Layer[i+384])),
                                       dtype=bool)
                reduced10List = np.array(np.append(reduced10List, (idx10Layer[i] and idx10Layer[i+128] and idx10Layer[i+256] and idx10Layer[i+384])),
                                       dtype=bool)
                reduced2List = np.array(np.append(reduced2List,(idx2Layer[i] and idx2Layer[i+128] and idx2Layer[i+256] and idx2Layer[i+384])),
                                       dtype=bool)
            ...
            # Check which neurons were dropped last round and is still in the unchangedlist
    ...
    def find_min(self, parameters: Parameters, results: List[Tuple[Weights, int]], idxList: List[int], rnd: int):
        ...
        for idx in idxList:
            ...
            b = np.maximum(np.maximum(reduced0List, reduced10List),reduced2List)
        ...
        # We take the average of both layer's min % diff in round 2 and 3 as the initial update threshol
            if (rnd == 2 or rnd ==3):
